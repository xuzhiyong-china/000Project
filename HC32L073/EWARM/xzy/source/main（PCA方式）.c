...
void Pca_IRQHandler(void)//PCA中断服务函数
{
  // M0P_PCA->CCON &= 0xBF; //清零，停止计数
 
//    Pca_StartPca(FALSE);//清零，停止计数
  //  debug=M0P_PCA->CNT;
  ...
  Pca_StartPca(FALSE);
    ...
    //需要将CCAPM0.CCIE置位时，这个标志位才会产生中断
    if((M0P_PCA->CCON) & 1)//CCF0 当出现匹配或捕获时，该位由硬件置位。
  // if((M0P_PCA->CCON) & 0x80)//PCA 计数器溢出标志
    {
        M0P_PCA->ICLR &= 0xfe;// PCA计数器模块0 比较/捕获标志位清除
       
    //if else 的顺序看是否有影响
        
       //切换占空比
       if( M0P_PCA->CCAP0_f.CCAP0==9)
        {
        /*
            debug+=1;debug+=1;debug+=1;debug+=1; debug+=1; debug+=1;debug+=1;debug+=1;debug+=1; debug+=1;  
            debug+=1;debug+=1;debug+=1;debug+=1; debug+=1; debug+=1;debug+=1;debug+=1;debug+=1; debug+=1; 
            debug+=1;debug+=1;debug+=1;debug+=1; debug+=1; debug+=1;debug+=1;debug+=1;debug+=1; debug+=1; 
            debug+=1;debug+=1;debug+=1;debug+=1; debug+=1; debug+=1;debug+=1;debug+=1;debug+=1; debug+=1;
            debug+=1;debug+=1;debug+=1;debug+=1; debug+=1; debug+=1;debug+=1;debug+=1;debug+=1; debug+=1;
            ...
            debug+=1;debug+=1;debug+=1;debug+=1; debug+=1; debug+=1;debug+=1;debug+=1;debug+=1; debug+=1;  
           */
            M0P_PCA->CCAP0_f.CCAP0 = 19;
        }
       else 
        //防止频率跳高--因为刚刚进入这里的占空比是9，延时一会再设置占空比，不然马上又进中断了，导致频率变高
            {   
             
            
            M0P_PCA->CCAP0_f.CCAP0 = 9;
            
        }
     //  M0P_PCA->ICLR &= 0x7f;//PCA 计数器溢出标志清除

    
     //   debug=0;

   //   M0P_PCA->CCON |= 0x40; //开启计数
        
//    Pca_StartPca(TRUE);  //开启计数
